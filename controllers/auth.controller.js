...
export const signin = async (req, res, next) => {
  ...
  if (!email || !password) {
    return next(handleError(StatusCodes.BAD_REQUEST, "Please provide all fields"));
  }
  ...
  try {
    ...
    res.cookie('access_token', token, { httpOnly: true }).status(200).json({ message: "User signed in successfully!", rest });
